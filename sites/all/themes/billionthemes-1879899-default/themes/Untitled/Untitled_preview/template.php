...
Untitled_preview_include_theme_files();

function Untitled_preview_include_theme_files() {
    ...
    $include_files[] = '/versions/'.Untitled_preview_get_major_version().'.x/theme-methods.php';
...
function Untitled_preview_get_major_version() {
    $tok = strtok(VERSION, '.');
    return (int)$tok[0];
}


function Untitled_preview_get_language($text_type = 'short') {
    global $language;
    switch ($text_type) {
        case 'full':
...
function Untitled_preview_get_direction() {
    global $language;
...
function Untitled_preview_render_template_from_includes($file_name, $args = array(), $folder = 'includes') {
    $filePath = Untitled_preview_normalize_path(dirname(__FILE__) .'/'. $folder . '/' . $file_name . '.php');
    include_once($filePath);
    return call_user_func_array('Untitled_preview_'.$file_name, $args);
}

function Untitled_preview_render_default_content($name = 'blog', $args = array(), $folder = 'includes') {
    Untitled_preview_autoinclude(dirname(__FILE__) .'/'. $folder, $name);
    $method_name = 'Untitled_preview_'.$name;
    if (function_exists($method_name))
        return call_user_func_array($method_name, $args);
    die('Method ' . $method_name . ' doesn\'t exist');
}

function Untitled_preview_autoinclude($folder, $filename){
    Untitled_preview_normalize_path($folder);
...
function Untitled_preview_normalize_path($path) {
    $root = ($path[0] === '/') ? '/' : '';
...
function Untitled_preview_generate_globals($vars, $preffix = '') {
...
function Untitled_preview_add_span_to_link($link) {
    $pattern = '/(.*?<a.*?>)(.*?)(<\/a>)/si';
    return preg_replace($pattern, '$1<span>$2</span>$3', $link);
}

function Untitled_preview_has_url_param($param) {
...
if (!function_exists('theme_get_image_path')) {
    function theme_get_image_path($image) {
        ...
        if (Untitled_preview_has_url_param('theme') && !file_exists(dirname(__FILE__) . '/images/' . $image)) {
...
function Untitled_preview_get_style_uri($name) {
    global $theme;
...
function Untitled_preview_render_notemplate_content($type) {
    $notemplate = '';
    switch ($type) {
        case 'page':
            ob_start();
