...
function Untitled_preview_menu_worker($content, $menu_id, $menu_class, $menu_item_class, $sub_menu_class='', $sub_item_class='', $submenu_popup = '',$show_sub_menus = false) {
    ...
    $output = Untitled_preview_menu_xml_parcer($output, $menu_id, $menu_class, $menu_item_class, $sub_menu_class, $sub_item_class, $submenu_popup, $show_sub_menus);
    /* Support Block Edit Link module */
    $result = str_replace('<!DOCTYPE root>', '', $output);
    return $result;
}

function Untitled_preview_menu_xml_parcer($content, $menu_id, $menu_class, $menu_item_class, $sub_menu_class, $sub_item_class, $submenu_popup, $show_sub_menus) {
    $parent_id = $menu_id . '-id';

    $doc = Untitled_preview_xml_document_creator($content, $parent_id);
    if ($doc === FALSE) {
    ...
    if (!empty($menu_item_class)) { // Don't use active class for first level <li> in Language menu etc.
        Untitled_preview_menu_set_active($doc, $menu_class);
    }
    Untitled_preview_menu_style_parcer($doc, $ul->childNodes, $menu_item_class, $sub_menu_class, $sub_item_class, $submenu_popup, $show_sub_menus);
...
function Untitled_preview_xml_document_creator($content, $parent_id) {
    $old_error_handler = set_error_handler('Untitled_preview_handle_xml_error');
    $dom = new DOMDocument();
    /* Support Block Edit Link module */
    $doc_content = <<< XML
...
function Untitled_preview_handle_xml_error($errno, $errstr, $errfile, $errline) {
...
function Untitled_preview_menu_style_parcer($doc, $elements, $menu_item_class, $sub_menu_class, $sub_item_class, $submenu_popup, $show_sub_menus) {
    $to_delete = array();
        ...
        if (is_a($element, "DOMElement") && ($element->tagName == "li")) {
            ...
            foreach ($children as $child) {
                ...
                elseif ($show_sub_menus && is_a($child, "DOMElement") && ($child->tagName == "ul")) {
                    ...
                    Untitled_preview_menu_style_parcer($doc, $child->childNodes, $sub_item_class, $sub_menu_class, $sub_item_class, $submenu_popup, $show_sub_menus);
    ...
    Untitled_preview_remove_elements($to_delete);
    return $elements;
}

function Untitled_preview_menu_set_active($doc, $menu_class) {
...
function Untitled_preview_remove_elements($elements_to_delete) {
