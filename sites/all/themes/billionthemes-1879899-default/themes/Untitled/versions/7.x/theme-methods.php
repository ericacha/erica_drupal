...
define('Untitled_DEFAULT', '__default');
define('Untitled_EXPORT_VERSION', 7);

function Untitled_preprocess_html(&$variables) {
    if (Untitled_has_url_param('theme') &&
    ...
    Untitled_set_template($variables, $node, 'html');
    
}

function Untitled_preprocess_block(&$variables) {
    $variables['theme_hook_suggestions'][] = 'block' . Untitled_DEFAULT;
...
function Untitled_preprocess_node(&$variables) {
    $variables['theme_hook_suggestions'][] = 'node' . Untitled_DEFAULT;
...
function Untitled_set_template(&$variables, $node, $base_name) {
    $variables['theme_hook_suggestions'][] =  $base_name . Untitled_DEFAULT;
    ...
    if ((count($variables['theme_hook_suggestions']) < 3) &&
        ...
        (in_array($base_name . Untitled_DEFAULT, $variables['theme_hook_suggestions']))) {
...
function Untitled_status_messages($variables) {
...
function Untitled_get_terms($content) {
    $result = array();
    $output = '';
...
function Untitled_get_links($content, $is_comment = false, $is_readmore = false, $class = '') {
    $result = '';
    ...
    foreach (array_keys($content['links']) as $name) {
        ...
        if (isset($content['links'][$name]['#links'])) {
            ...
            if (is_array($links)) {
                $output = Untitled_get_links_output($links, $is_comment, $is_readmore, $class);
                if (!empty($output)) {
...
function Untitled_get_links_output($links, $is_comment = false, $is_readmore = false, $add_class = '') {
    $output = '';
...
function Untitled_menu_link(array $variables) {
    $element = $variables['element'];
    $sub_menu = '';
    ...
    $output = Untitled_add_span_to_link($output);
