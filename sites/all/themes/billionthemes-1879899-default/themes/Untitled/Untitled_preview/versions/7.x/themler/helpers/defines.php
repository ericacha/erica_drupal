...
function get_theme_dir($theme_name) {
    return FilesHelper::normalize_path(DRUPAL_ROOT).'/'.drupal_get_path('theme', $theme_name);
}
...
function get_modules_dir() {
    $modules_dir = FilesHelper::normalize_path(DRUPAL_ROOT).'/'.drupal_get_path('module', 'themler');
    $modules_dir = substr($modules_dir, 0, strpos($modules_dir, '/themler'));
    return $modules_dir;
}

function get_files_dir() {
    return drupal_realpath(file_default_scheme() . '://');
}