...
define('Untitled_DEFAULT', '-default');
define('Untitled_EXPORT_VERSION', 6);

function Untitled_preprocess_page(&$variables) {
    if (Untitled_EXPORT_VERSION !== Untitled_get_major_version()) {
    ...
    Untitled_set_template($variables, $node, 'page');
...
function Untitled_preprocess_block(&$variables) {
    $variables['template_files'][] = 'block' . Untitled_DEFAULT;
...
function Untitled_preprocess_node(&$variables) {
    // Comments not working with for a while - only from theme root only
    $variables['template_files'][] = 'node' . Untitled_DEFAULT;
    $variables['template_files'][] = 'node-' . $variables['node']->type;
}

function Untitled_set_template(&$variables, $node, $base_name) {
    $variables['template_files'][] = $base_name. Untitled_DEFAULT;
    $arg0 = arg(0);
    $arg1 = arg(1);
    $arg2 = arg(2);
    ...
    if (count($variables['template_files']) < 3 && 
        ...
        in_array($base_name . Untitled_DEFAULT, $variables['template_files'])) {
...
function Untitled_preprocess_search_theme_form(&$vars, $hook) {
    Untitled_set_themler_classes($vars);
}

function Untitled_preprocess_search_block_form(&$vars, $hook)  {
    Untitled_set_themler_classes($vars, true);
}

function Untitled_form($element) {
...
function Untitled_status_messages($display = NULL) {
    $output = '';
...
function Untitled_set_themler_classes(&$vars, $is_widget = false) {
...
function Untitled_get_terms($node) {
    $result = '';
...
function Untitled_get_links($links, $is_comment = false, $is_readmore = false, $add_class = '') {
    $output = '';
    ...
    if (!empty($links)) {
        ...
        foreach ($links as $key => $link) {
            ...
            $link_output = Untitled_get_link_output($link);
...
function Untitled_get_link_output($link) {
    $output = '';
    // Is the title HTML?
...
function Untitled_is_links_set($links) {
    $size = sizeof($links);
    if ($size == 0) {
        return FALSE;
    }
...
function Untitled_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
    ...
    $output = Untitled_add_span_to_link($link);
